...
const mapAgent = (a) => {
    if (!a) return null;
    return {
        id: a.id || a.agent_id || a._id,
        name: a.name,
        description: a.description,
        type: a.type,
        ticker: a.ticker,
        bondingCurveProgress: a.bonding_curve_progress || a.bondingCurveProgress,
        holders: a.holders,
        status: a.status,
...
// --- LOGS & STATUS ---
app.post('/agents/logs', authenticateToken, async (req, res) => {
    const { agentName, level, message, timestamp } = req.body;
    try {
        const logEntry = {
            agentName,
            level,
...
app.patch('/agents/status', authenticateToken, async (req, res) => {
    const { name, status } = req.body;
    try {
        await supabase.from('agents').update({ status }).eq('name', name).eq('user_id', req.user.id);
        res.json({ success: true });
    } catch (e) {
        res.status(500).json({ error: e.message });
    }
...
// SPA catch-all (Removed for Vercel - handled by vercel.json)
